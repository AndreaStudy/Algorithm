...
mw_idx = 0  # 가장 긴 가로길이의 인덱스를 저장할 변수
mh_idx = 0  # 가장 긴 높이의 인덱스를 저장할 변수
arr = []  # 변의 정보들을 저장할 리스트
for i in range(6):
...
product = 1  # 곱을 저장할 변수
for i in arr:  # 입력받은 변들 중에
    if i not in idx_list:  # 만약 새로운 리스트에 저장한 변 4개 중에 없다면 빈 사각형이므로
